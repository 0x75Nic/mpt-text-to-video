...
- **画面具体、有动作**：「阳光洒进窗边小院」优于「这里很美」；给素材检索留画面锚点。
- **节奏均匀**：每行约 3–10 秒，太长会被自动裁、太短会重复素材。
- **不要**写镜头术语（「近景」「转场」），MPT 不解析；只写自然语言旁白。
- 示例（城市咖啡馆探店）：
  ```
  清晨，城市街角的咖啡馆刚开门，阳光斜斜洒在木质吧台上。
  店主手冲一杯单品，热气在晨光里轻轻升起。
  窗边绿植爬满木架，风一吹全是咖啡香。
  客人推开玻璃门坐下，远处的街景在窗外缓缓流动。
  这就是街角咖啡馆，一个可以慢下来的地方。
  ```
...
## 进阶
- 可做**定时自动化**：定期生成一条主题短视频，用 automation_update 创建 once/recurring 任务，prompt 写「用 mpt-text-to-video 生成一个关于 XX 的竖屏短视频」。
- **行业专属模板**：把分镜套路（如街景、花架、咖啡、自然、音乐等元素）固化进本技能，生成更贴近你的业务调性。
